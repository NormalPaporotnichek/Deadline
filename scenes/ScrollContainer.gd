...
export(float, 0.1, 1, 0.1) var card_scale = 1
export(float, 0.1, 1.5, 0.1) var card_current_scale = 1.3
...
func _on_Scene1Card_gui_input(event):
	if event.pressed:
			get_tree().change_scene("res://Main.tscn")

func _on_Scene2Card_gui_input(event):
	if event.pressed:
